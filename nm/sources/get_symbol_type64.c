...
static char get_global_symbol_type64(Elf64_Shdr *shdr, Elf64_Sym symbol,
...
const char *symbol_name)
{
    char symbol_type = get_global_symbol_type64(elf->shdr64,
    symbol, symbol_name);
    ...
    if (ELF64_ST_BIND(symbol.st_info) == STB_WEAK)
        ...
        if (symbol_type == '?' && is_symbol_array(symbol_name))
            return 'd';
        if (symbol_type == '?' && elf->ehdr64->e_type == ET_REL)
            return 'n';
        if (symbol_type != '?'
