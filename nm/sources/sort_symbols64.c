...
static void get_unsorted_symbols64(elf_t *elf, size_t symnb, Elf64_Sym *symtab,
uint8_t *strtab)
{
    char buffer[128] = {0};
    ...
    for (size_t i = 0; i < symnb; i++)
    {
        ...
        {
            sprintf(buffer, "%s %c %016lx %d",
            &strtab[symtab[i].st_name],
            get_symbol_type64(elf, symtab[i],
...
uint8_t *strtab)
{
    get_unsorted_symbols64(elf, symnb, symtab, strtab);
