...
static bool is_valid_section32(Elf32_Shdr shdr)
{
    if (shdr.sh_size
...
static void dump_section_content32(Elf32_Shdr shdr, elf_t *elf,
bool is_debug_str)
{
    uint8_t *line_data;
    size_t line_size;
    ...
    for (size_t i = 0; i < shdr.sh_size; i += 16)
    {
        ...
        line_data = (uint8_t *)elf->ehdr32 + shdr.sh_offset + i;
...
void dump_section_info32(elf_t *elf, const char *filename)
{
    ...
    for (int i = 0; i < elf->ehdr32->e_shnum; i++) {
        ...
        if (section_name && is_valid_section32(elf->shdr32[i])) {
            is_debug_str = false;
            printf("Contents of section %s:\n", section_name);
            set_debug_flag(&is_debug_str, filename, section_name);
            dump_section_content32(elf->shdr32[i], elf, is_debug_str);
        }
    }
}